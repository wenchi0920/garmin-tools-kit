...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("-v", "--verbosity", action="count", default=0, help="增加輸出與日誌詳細度，儲存更多中間檔案")
    ...
    parser.add_argument("--env-file", help="使用 env file 優先使用 這個來取得 username , password")
    parser.add_argument("-c", "--count", default="1", help="要下載的最近活動數量，或輸入 'all'（預設值：1）")
    parser.add_argument("-sd", "--start_date", help="開始抓取活動的日期（包含當日）。格式範例：2023-07-31")
    parser.add_argument("-ed", "--end_date", help="結束抓取活動的日期（包含當日）。格式範例：2023-07-31")
    parser.add_argument("-f", "--format", choices=["gpx", "tcx", "original", "json"], default="gpx", help="匯出格式；可選 'gpx'、'tcx'、'original' 或 'json'（預設值：'gpx'）")
    parser.add_argument("-d", "--directory", default="./data", help="匯出目錄路徑（預設值：'./data'）")
    parser.add_argument("-ot", "--originaltime", action="store_true", help="將下載（或解壓縮後）的檔案時間設定為活動開始時間，注意時區問題")
    parser.add_argument("--desc", nargs="?", const=True, help="將活動描述附加至下載的檔案名稱；若提供數字則限制字數長度")
    parser.add_argument("-ss", "--session", default=".garth", help="啟用從指定目錄讀取與儲存 SSO 認證資訊")
...
def main() -> None:
    ...
    # 最後才是互動式輸入
    username = username or input("Garmin Connect 使用者名稱: ")
    password = password or getpass.getpass("Garmin Connect 密碼: ")
    ...
    try:
        ...
        activities: List[Dict[str, Any]] = client.list_activities(count=args.count, start_date=args.start_date, end_date=args.end_date)
        
        if not activities:
            logger.warning("找不到任何活動。")
            return
        ...
        # 使用 tqdm 顯示下載進度
        for activity in tqdm(activities, desc="下載活動", unit="個"):
            activity_id = activity["activityId"]
            activity_name = activity["activityName"]
            start_time = activity["startTimeLocal"]
            
            logger.debug(f"正在下載活動 {activity_id}: {activity_name} ({start_time})")
