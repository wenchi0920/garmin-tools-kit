...
2026-04-06: 1.5.3 - 🚀 **重構 `race-event`：優化摘要顯示並簡化參數。**
    ...
    - 優化預設輸出檔名邏輯。
2026-04-06: 1.5.4 - 🚀 **優化 `race-event` 摘要表格欄位。**
    - 新增「距離」欄位，並實作中英文混排自動對齊。
    - 調整欄位順序：時間、分類、賽事名稱、距離、類型。
2026-04-06: 1.5.5 - 🚀 精確優化 race-event 摘要視覺佈局。
    - 依照範例調整表格分隔線、間距與 Emoji 分類顯示位置。
    - 強化中英文混排對齊精確度。
2026-04-24: 1.5.6 - 🚀 **優化 `summary` 命令資料欄位與格式。**
    - 依照需求調整 HRV、睡眠分數、能量、完備度與血壓之資料來源與顯示格式。
    - 修正表格重複輸出之 bug。
    - 確保 `None` 值統一顯示為 `--` 並優化欄位對齊。
"""
import argparse
import sys
