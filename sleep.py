#!/usr/bin/env python3
"""
Purpose: Garmin Connect 睡眠紀錄與睡眠分數抓取工具。
Author: Gemini CLI
Changelog:
2026-03-08: 1.0.0 - 初始版本，支援睡眠紀錄、睡眠階段與睡眠分數抓取。
"""
import argparse
import getpass
import os
import sys
import json
from datetime import datetime, date, timedelta
from typing import Dict, Optional, Any, List, Union
from loguru import logger
from tqdm import tqdm

# Import SleepClient from client package
from client import SleepClient, VERSION

def setup_logging(verbosity: int) -> None:
    logger.remove()
    if verbosity == 0:
        level = "INFO"
    elif verbosity == 1:
        level = "DEBUG"
    else:
        level = "TRACE"
    logger.add(sys.stderr, level=level, format="<green>{time:YYYY-MM-DD HH:mm:ss}</green> | <level>{level: <8}</level> | <cyan>{message}</cyan>")

def format_seconds(seconds: int) -> str:
    hours, remainder = divmod(seconds, 3600)
    minutes, _ = divmod(remainder, 60)
    return f"{hours}時{minutes}分"

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Garmin Connect 睡眠紀錄抓取工具")
    parser.add_argument("--version", action="version", version=f"%(prog)s {VERSION}")
    parser.add_argument("-v", "--verbosity", action="count", default=0, help="增加輸出詳細度")
    parser.add_argument("--username", help="Garmin Connect 使用者名稱")
    parser.add_argument("--password", help="Garmin Connect 密碼")
    parser.add_argument("--env-file", nargs="?", const=".env", help="使用 env file 取得帳密")
    
    # Selection args
    parser.add_argument("-d", "--date", help="指定日期 (YYYY-MM-DD)，預設為今天", default=date.today().isoformat())
    parser.add_argument("-sd", "--start_date", help="開始日期 (YYYY-MM-DD)")
    parser.add_argument("-ed", "--end_date", help="結束日期 (YYYY-MM-DD)")
    
    # Operation args
    parser.add_argument("-o", "--output", help="輸出 JSON 檔案路徑")
    parser.add_argument("-ss", "--session", default=".garth", help="SSO 認證目錄")
    parser.add_argument("--summary", action="store_true", help="僅顯示文字摘要，不輸出完整 JSON")
    
    return parser.parse_args()

def load_env_file(file_path: str) -> Dict[str, str]:
    env_data: Dict[str, str] = {}
    if not os.path.exists(file_path): return env_data
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if not line or line.startswith("#") or "=" not in line: continue
                key, value = line.split("=", 1)
                env_data[key.strip()] = value.strip().strip("'").strip("\"")
    except Exception as e:
        logger.error(f"讀取 env-file 錯誤: {e}")
    return env_data

def print_sleep_summary(data: Any) -> None:
    dto = data.get("dailySleepDTO", {})
    if not dto: return
    
    calendar_date = dto.get("calendarDate")
    total_time = format_seconds(dto.get("sleepTimeSeconds", 0))
    deep = format_seconds(dto.get("deepSleepSeconds", 0))
    light = format_seconds(dto.get("lightSleepSeconds", 0))
    rem = format_seconds(dto.get("remSleepSeconds", 0))
    awake = format_seconds(dto.get("awakeSleepSeconds", 0))
    
    score_data = dto.get("sleepScores", {}).get("overall", {})
    score = score_data.get("value", "N/A")
    qualifier = score_data.get("qualifierKey", "N/A")
    feedback = dto.get("sleepScoreFeedback", "N/A")
    
    print("-" * 40)
    print(f"日期: {calendar_date}")
    print(f"睡眠分數: {score} ({qualifier})")
    print(f"總睡眠時間: {total_time}")
    print(f"睡眠分析: 深層 {deep} | 淺層 {light} | REM {rem} | 清醒 {awake}")
    print(f"系統評語: {feedback}")
    print("-" * 40)

def main() -> None:
    args = parse_args()
    setup_logging(args.verbosity)
    
    username, password = args.username, args.password
    env_file_path = args.env_file or (".env" if os.path.exists(".env") else None)
    if env_file_path:
        env_vars = load_env_file(env_file_path)
        username = username or env_vars.get("GARMIN_USERNAME") or env_vars.get("USERNAME")
        password = password or env_vars.get("GARMIN_PASSWORD") or env_vars.get("PASSWORD")
    
    if not username: username = input("Username: ")
    if not password: password = getpass.getpass("Password: ")
    
    try:
        client = SleepClient(email=username, password=password, session_dir=args.session)
        results = {}

        if args.start_date:
            end_date = args.end_date or date.today().isoformat()
            logger.info(f"正在抓取睡眠資料列表: {args.start_date} 至 {end_date}")
            sleep_list = client.get_sleep_data_range(args.start_date, end_date)
            results["data"] = [s.model_dump(mode="json") for s in sleep_list]
            logger.info(f"成功抓取 {len(sleep_list)} 筆資料。")
        else:
            target_date = args.date
            logger.info(f"正在抓取日期 {target_date} 的睡眠資料...")
            sleep_data = client.get_sleep_data(target_date)
            if sleep_data:
                results["data"] = sleep_data.model_dump(mode="json")
            else:
                logger.warning(f"找不到 {target_date} 的睡眠數據。")

        if not results or not results.get("data"):
            logger.error("未獲取到任何資料。")
            return

        # Output logic
        if args.summary:
            if isinstance(results["data"], list):
                for d in results["data"]: print_sleep_summary(d)
            else:
                print_sleep_summary(results["data"])
        
        output_str = json.dumps(results, indent=4, ensure_ascii=False)
        if args.output:
            with open(args.output, "w", encoding="utf-8") as f:
                f.write(output_str)
            logger.success(f"結果已儲存至: {args.output}")
        elif not args.summary:
            print(output_str)

    except Exception as e:
        logger.error(f"執行失敗: {e}")
        if args.verbosity > 0: logger.exception("Stack trace:")
        sys.exit(1)

if __name__ == "__main__":
    main()
